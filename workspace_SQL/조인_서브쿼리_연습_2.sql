...
(1, '소소리', '2024-03-20 12:15:00', '2024-03-24 12:15:00');
...
FROM TEST_BOOK TB INNER JOIN RENTAL R
ON TB.BOOK_ID = R.BOOK_ID;

SELECT TITLE, AUTHOR, (SELECT BORROWER_NAME
								FROM RENTAL
								WHERE RENTAL.BOOK_ID = test_book.BOOK_ID) BORROWER_NAME,
								(SELECT RENTAL_DATE
								FROM RENTAL
								WHERE RENTAL.BOOK_ID = test_book.BOOK_ID) RENTAL_DATE
FROM TEST_BOOK;
...
SELECT TITLE AS '책제목'
		 , (SELECT COUNT(RENTAL_ID) FROM RENTAL WHERE BOOK_ID = TEST_BOOK.BOOK_ID) AS '대출횟수'
		 , CASE
		 	WHEN (SELECT COUNT(RENTAL_ID)
			 		FROM RENTAL
					 WHERE BOOK_ID = test_book.BOOK_ID) THEN '대출기록있음'
			ELSE '대출이력없음'
			END AS '대출여부'
FROM test_book;
